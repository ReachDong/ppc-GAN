...
def discriminator(x, y, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        dense1 = tf.layers.dense(cat1, 1200, kernel_initializer=w_init)
...
G_z = generator(noise_tensor, encode_before, isTrain)
...
G_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_fake_logits, labels=tf.ones([mb_size, 1])))
...
def my_next_feed_dict(index_num,feature_df,label_df):
    #5# 需要重写
    before = feature_df.loc[index_num].values#  dmso expression
...
if __name__=="__main__":
    ...
    # split_index = feature.shape[0]
    # feature = feature.append(label,ignore_index=True)
    # new_feature = pd.DataFrame(columns=feature.columns)
    # for col in feature.columns:
    #     x_mean = feature[col].mean()
    #     x_std = feature[col].std()
    #     new_feature[col] = (feature[col]-x_mean)/x_std
    # feature = new_feature.iloc[0:split_index,:]
    # label = new_feature.iloc[split_index:,:]
    ...
    dic.to_csv('../../data/lincs_data/out/ans/standard_try'+'12_23_'+"predict_v2"+'.csv',header=True)
